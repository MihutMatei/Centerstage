...
@TeleOp(name="RaulWTF", group="Linear Opmode")
@Config


public class ReverseKinematicsTest extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        while (opModeIsActive() && !isStopRequested()) {
            double dist=robot.distanceSensor.getDistance(DistanceUnit.INCH) + robot.htarget*Math.tan(Math.toRadians(30));
            double angle = (Math.acos((robot.htarget-dist)/robot.lenbrat)-Math.toRadians(45))/robot.sqrt2;
            double extension =robot.sqrt2*(dist - Math.cos(angle)*robot.lenbrat);
            /*-------------P2 CONTROLS-------------------------------------*/
...
//            if(gamepad1.start) robot.drone_reset();
            ...
            switch (sliderstate){
                case AUTO:
                    if(gamepad2.dpad_down) robot.setHtarget(12);
                    if(gamepad2.dpad_up) robot.setHtarget(30);
                    if(gamepad2.dpad_left) robot.setHtarget(24);
